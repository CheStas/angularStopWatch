...
export class StopwatchComponent implements OnInit {
  ...
  lap():void {
      ...
      this.laps.unshift({
          ...
          colorPrev: (this.lapInfo.current - this.lapInfo.prev) < 0 ? '-' + this.appService.beautyTime(-1 * (this.lapInfo.current - this.lapInfo.prev)) : this.appService.beautyTime(this.lapInfo.current - this.lapInfo.prev)
      })

      this.laps.forEach((el)=>{
          el.colorAvg = (el.currentRaw - (this.time/this.lapInfo.count)) < 0 ? '-' + this.appService.beautyTime(-1*(el.currentRaw - (this.time/this.lapInfo.count))) : this.appService.beautyTime(el.currentRaw - (this.time/this.lapInfo.count))
      })
  }
